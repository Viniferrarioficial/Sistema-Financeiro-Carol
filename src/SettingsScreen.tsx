...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ onLogout }) => {
  return (
    <div className="flex flex-col min-h-screen bg-background-light w-full pb-24 lg:pb-8">
      {/* Header */}
      <div className="flex items-center p-4 sticky top-0 bg-background-light/80 backdrop-blur-md z-10 w-full max-w-7xl mx-auto">
        <button className="text-slate-900 p-2 hover:bg-primary/10 rounded-full transition-colors cursor-pointer">
          <ArrowLeft size={24} />
        </button>
        <h2 className="text-lg font-extrabold flex-1 ml-2 text-slate-900">Configurações de Perfil</h2>
        <button className="text-primary font-extrabold text-sm px-4 py-2 hover:bg-primary/10 rounded-xl cursor-pointer">
          Salvar
        </button>
      </div>
      ...
      <div className="w-full max-w-3xl mx-auto flex flex-col pt-4">
        ...
        <div className="flex p-6 flex-col items-center gap-4">
          <div className="relative">
            ...
            <button className="absolute bottom-0 right-0 bg-primary text-white p-2.5 rounded-full shadow-lg border-2 border-white hover:scale-110 transition-transform cursor-pointer">
              <Camera size={16} />
            </button>
          </div>
